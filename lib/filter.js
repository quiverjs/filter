...
var metaFilter = function(customFilter, builder) {
  var filter = function(config, handler, callback) {
    customBuilder(copyObject(config), function(err, custom) {
      if(err) return callback(err)

      var customFilter = filterBuilder(custom)
      customFilter(config, handler, callback)
    })
  }
